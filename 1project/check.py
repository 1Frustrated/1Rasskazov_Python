...
word_label = Label(root, text="", font=("Bahnschrift", 24))
...
# Метка для отображения виселицы (рисунок)
...
# Метка для сообщений
message_label = Label(root, text="", font=("Bahnschrift", 12), fg="red")
message_label.pack()
...
def button_clicked():
    ...
    if len(guess) != 1:
        message_label.config(text="Введите одну букву.")
        return
    if not guess.isalpha():
        message_label.config(text="Введите букву русского алфавита.")
        return
    if guess in guessed_letters:
        message_label.config(text="Эта буква уже была.")
        return

    guessed_letters.add(guess)

    if guess in word_letters:
        message_label.config(text="Верно!")
    else:
        tries -= 1
        message_label.config(text=f"Неверно! Осталось попыток: {tries}")

    update_display()

    if all(letter in guessed_letters for letter in word_letters):
        message_label.config(text=f"Поздравляем! Вы угадали слово: {word}")
        guess_btn.config(state=DISABLED)
        entry.config(state=DISABLED)
    elif tries == 0:
        message_label.config(text=f"Вы проиграли. Было загадано слово: {word}")
        guess_btn.config(state=DISABLED)
        entry.config(state=DISABLED)
